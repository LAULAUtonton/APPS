...
1. Create a Google Sheet and name a tab **FlightPoints**.
...
5. Deploy as **Web app** with access set to **anyone with link**.
6. Put the web app URL into `.env` as `VITE_APPS_SCRIPT_URL`.
7. If the URL is empty, the app still works with localStorage fallback and shows a warning.
...
- `src/utils.js` - local date and weekly/redeem helper utilities
